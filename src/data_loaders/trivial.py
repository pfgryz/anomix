...
def get_blob_datasets() -> list[DatasetWrapperBase]:
    return [
        DatasetWrapperBase("one_blob_and_noise_2d", *combine_datasets([
            create_blobs_data([
                (0, 0)
            ], 1.0, 100, 0),
            create_uniform_anomalies(20, -5, 5, 1)
        ])),
        DatasetWrapperBase("two_blobs_and_noise", *combine_datasets([
            create_blobs_data([
                (-0.5, -0.5), (0.5, 0.5)
            ], 1.0, 100, 0),
            create_uniform_anomalies(20, -5, 5, 1)
        ])),
        DatasetWrapperBase("two_moons", *combine_datasets([
